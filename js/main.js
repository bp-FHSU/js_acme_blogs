...
const toggleCommentSection = (postId) => {
    if (!postId) return
    const postSelection = document.querySelector(`section[data-post-id='${postId}']`);
...
const toggleCommentButton = (postId) => {
    if (!postId) return;
    const buttonSelection = document.querySelector(`button[data-post-id='${postId}']`);
...
const getUsers = async () => {
    try {
        const JSONtext = await (await fetch("https://jsonplaceholder.typicode.com/users")).json()
        return JSONtext
    }
    catch (err) {
...
const getUserPosts = async (userId) => {
    if (!userId) return;
    try {
        const JSONtext = await (await fetch(`https://jsonplaceholder.typicode.com/posts?userId=${userId}`)).json()
        return JSONtext
    }
    catch (err) {
        console.error(err.stack);
...
const getUser = async (userId) => {
    if (!userId) return;
    try {
        const JSONtext = await (await fetch(`https://jsonplaceholder.typicode.com/users/${userId}`)).json()
        return JSONtext
    }
    catch (err) {
        console.error(err.stack);
...
const getPostComments = async (postId) => {
    if (!postId) return;
    try {
        const JSONtext = await (await fetch(`https://jsonplaceholder.typicode.com/comments?postId=${postId}`)).json()
        return JSONtext
    }
    catch (err) {
        console.error(err.stack);
...
const createPosts = async (postJSON) => {
    ...
    for (const post of postJSON) {
        ...
        const credit = createElemWithText('p', `Author: ${author.name} with ${author.company.name}`)
        ...
        article.append(title, body, id, credit, catchphrase, commentButton, section)
        fragment.append(article)
    }
    return fragment
}
...
const displayPosts = async (posts) => {
    ...
    if (posts) {
        ...
        const element = createElemWithText("p", "Select an Employee to display their posts.", "default-text")
        main.append(element)
        return element
    }
}
